...
export default async function ExpensesPage() {
  ...
  const [{ data: categories }, { data: paymentMethods }, { data: transactions }] =
    await Promise.all([
      ...
      supabase
        ...
        .select(
          "id, description, amount_cents, occurred_on, financial_categories(id, name), suppliers(name)",
        )
        .eq("type", "EXPENSE")
        .is("deleted_at", null)
  ...
  return (
    <div>
      <h1 className="mb-1 text-2xl font-semibold tracking-tight">Despesas</h1>
          ...
          <table className="w-full min-w-[720px] text-sm">
            <thead>
              <tr className="border-border border-b border-dashed text-left">
                ...
                <th className="label-tag py-2 pr-4">Fornecedor</th>
              ...
              {transactions.map((t) =>
                isAdmin ? (
                  ...
                  <tr key={t.id} className="border-border border-b border-dashed last:border-0">
                    ...
                    <td className="py-2.5 pr-4">{t.suppliers?.name ?? "—"}</td>
